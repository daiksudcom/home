...
function labelsForPaths(paths) {
  const predicates = {
    ...
    build: (path) =>
      /^(?:package\.json|pnpm-lock\.yaml|pnpm-workspace\.yaml|astro\.config\.[^/]+|tsconfig\.json|wrangler\.[^/]+)$/.test(
        path,
      ),
...
test('branch fixtures produce the intended synchronized labels', () => {
  const fixtures = [
    ['breaking-change/feat/routing', ['BREAKING CHANGE', 'feat']],
    ['deprecated/fix/legacy-redirect', ['DEPRECATED', 'fix']],
    ['feat/hero-section', ['feat']],
    ['perf/asset-inlining', ['perf']],
    ['fix/navigation-overflow', ['fix']],
    ['revert/bad-change', ['revert']],
    ['docs/operating-model', ['docs']],
    ['refactor/layout-components', ['refactor']],
    ['style/format', ['style']],
    ['test/policy', ['test']],
...
// repository, never by a branch here. A branch that merely looks like a release must stay inert.
...
test('path-only rules use one combined all-files glob', () => {
  ...
  assert.deepEqual(patterns, [
    ...
    '{package.json,pnpm-lock.yaml,pnpm-workspace.yaml,astro.config.*,tsconfig.json,wrangler.*}',
    '.github/{workflows/**,scripts/**,*.yml}',
  ]);

  const fixtures = [
    [['README.md', 'docs/adr/0001.md'], ['docs']],
    [['docs/features/deploy.feature'], ['docs']],
    [['README.md', 'src/pages/index.astro'], []],
    [['src/pages/index.astro'], []],
    [['package.json', 'pnpm-lock.yaml'], ['build']],
    [['wrangler.jsonc', 'astro.config.mjs'], ['build']],
    [['package.json', 'README.md'], []],
    [['.github/workflows/ci.yml', '.github/scripts/policy.mjs'], ['ci']],
    [['.github/dependabot.yml', '.github/labeler.yml'], ['ci']],
    [['.github/workflows/ci.yml', 'package.json'], []],
    // A test file under .github/scripts is both a test and automation, and neither label
    // carries version impact, so both applying is harmless.
    [['.github/scripts/policy.test.mjs'], ['test', 'ci']],
